...
def _build_spec_from_markdown(
    ...
    arousal_gate = _section(body, "Arousal Gate")
    va_anchors = _section(body, "VA Anchors (1-9)")
    reasoning_principle = "\n".join(
        part
        for part in (
            arousal_gate,
            ...
            _section(body, "Reasoning Principle"),
            _section(body, "Mapping Principle"),
        )
        if part
    )
    ...
    worked_example = _section_any(body, ["Worked Examples", "Worked Example"])
...
def _parse_routing_card(text: str) -> dict[str, list[str]]:
    ...
    for line in text.splitlines():
        ...
        # Fable cards may annotate a gate label, e.g. "REJECT (route to no_specialized_skill)".
        if label.startswith("REJECT ") or label.startswith("REJECT("):
            label = "REJECT"
        elif label.startswith("REQUIRED ") or label.startswith("REQUIRED("):
            label = "REQUIRED"
        elif label.startswith("NEAR MISS ") or label.startswith("NEAR MISS("):
            label = "NEAR MISS"
