...
class PsychologyVAPipeline:
    ...
    def analyze(self, image_input: ImageInput, user_context: str = "") -> VAAnalysisResult:
        direct_result = self.analyze_direct(image_input=image_input, user_context=user_context)
        route = self.route_skill(image_input=image_input, user_context=user_context)
        return self.analyze_with_route(
    ...
    def analyze_with_route(
        ...
        route: RouteDecision,
        user_context: str = "",
        direct_result: VAAnalysisResult | None = None,
    ) -> VAAnalysisResult:
        if route.skill_id == NO_SPECIALIZED_SKILL_ID:
            return direct_result or self.analyze_direct(image_input=image_input, user_context=user_context)
        ...
        response = self.llm_client.complete_json(
            ...
            user_prompt=build_analysis_user_prompt(
                ...
                user_context=user_context,
                direct_valence_score=direct_result.valence_score,
                direct_arousal_score=direct_result.arousal_score,
            ),
            image_payload=image_input.to_llm_payload(),
        )
        _validate_va_mapping_payload(response, direct_result)
...
def _validate_va_mapping_payload(payload: dict[str, Any], direct_result: VAAnalysisResult) -> None:
    applicability = str(payload.get("applicability") or "").strip().lower()
    expected_weights = {"strong": 0.85, "partial": 0.55, "weak": 0.25}
    if applicability not in expected_weights:
        raise ValueError("VA-mapping output must use applicability: strong, partial, or weak")

    direct_va = payload.get("direct_va")
    candidate = payload.get("skill_candidate")
    if not isinstance(direct_va, dict) or not isinstance(candidate, dict):
        raise ValueError("VA-mapping output must include direct_va and skill_candidate objects")

    expected_weight = expected_weights[applicability]
    weight = _required_float_in_range(payload, "blend_weight", low=0.0, high=1.0)
    if abs(weight - expected_weight) > 1e-6:
        raise ValueError(
            f"blend_weight {weight} does not match fixed {applicability} weight {expected_weight}"
        )

    for dimension in ("valence", "arousal"):
        key = f"{dimension}_score"
        supplied_direct = _required_float_in_range(direct_va, key, low=1.0, high=9.0)
        actual_direct = float(getattr(direct_result, key))
        if abs(supplied_direct - actual_direct) > 1e-6:
            raise ValueError(f"direct_va.{key} differs from the supplied Direct baseline")

        candidate_score = _required_float_in_range(candidate, key, low=1.0, high=9.0)
        final_score = _required_float_in_range(payload, key, low=1.0, high=9.0)
        expected_final = actual_direct + weight * (candidate_score - actual_direct)
        if abs(final_score - expected_final) > 0.06:
            raise ValueError(
                f"{key}={final_score} violates Direct-anchor formula; expected {expected_final:.3f}"
            )
