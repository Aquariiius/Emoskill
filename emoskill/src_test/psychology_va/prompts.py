...
VA_MAPPING_EXPERIMENT_INSTRUCTIONS = (
    "This is the isolated VA-mapping ablation. The selected skill's VA Mapping and Direct-Anchor Rule are binding.\n"
    "1. Use the supplied Direct VA scores exactly; do not re-estimate or rewrite them.\n"
    "2. Complete the observable reasoning steps before choosing a skill_candidate on the shared 1-9 scale.\n"
    "3. Set blend_weight only from applicability: strong=0.85, partial=0.55, weak=0.25.\n"
    "4. Compute each final score as direct + blend_weight * (skill_candidate - direct).\n"
    "5. Put the computed final values in the top-level valence_score and arousal_score fields.\n"
    "Do not choose a different weight because it gives a more plausible score.\n"
)
...
def _routing_preference_rules() -> str:
    return (
        ...
        "- Prefer todorov-face-evaluation only for a clear, face-dominant, low-context portrait with neutral, resting, or mild expression; strong expression or event context rejects it.\n"
...
def build_analysis_system_prompt(skill_spec: PsychologySkillSpec) -> str:
    ...
    return (
        ...
        f"{VA_MAPPING_EXPERIMENT_INSTRUCTIONS}"
        ...
        '  "direct_va": {"valence_score":5.0,"arousal_score":5.0},\n'
        '  "skill_candidate": {"valence_score":5.0,"arousal_score":5.0},\n'
        '  "blend_weight": 0.55,\n'
        ...
        '  ,"mapping_trace": ["Direct -> skill candidate -> weighted final calculation"]\n'
        "}\n"
    )
...
def build_analysis_user_prompt(
    ...
    route_reason: str,
    user_context: str,
    direct_valence_score: float | None = None,
    direct_arousal_score: float | None = None,
) -> str:
    direct_text = (
        f"Supplied Direct VA (copy exactly): valence_score={direct_valence_score}, "
        f"arousal_score={direct_arousal_score}.\n"
        if direct_valence_score is not None and direct_arousal_score is not None
        else "No Direct VA was supplied; this call is schema-only and must not be used for final comparison.\n"
    )
    return (
